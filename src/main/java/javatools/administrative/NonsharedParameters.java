...
public class NonsharedParameters implements Cloneable {
  ...
  public static FinalSet<String> no = new FinalSet<String>(new String[] { "inactive", "off", "false", "no", "none" });
  ...
  public NonsharedParameters(File iniFile, String localPath) throws ParameterFileException {
    if (localPath != null) basePath = localPath.endsWith("/") ? localPath : localPath + "/";
    init(iniFile);
  };
  ...
  public Integer getInteger(String s, Integer defaultValue) throws UndefinedParameterException {
    if (isDefined(s)) return Integer.parseInt(get(s));
  ...
  public Map<String, String> getMap(String s) throws UndefinedParameterException {
    ...
    for (String entry : get(s).split("\\s*,\\s*")) {
      ...
      if (entrypair.length < 2) return null;
  ...
  public String getMapEntry(String s, String key) throws UndefinedParameterException {
    ...
    for (String entry : get(s).split("\\s*,\\s*")) {
      String[] entrypair = entry.split("\\s*-->\\s*");
      if (entrypair.length < 2) return null;
      if (entrypair[0].toLowerCase().equals(key.toLowerCase())) return entrypair[1];
    }
    return null;
  }
  ...
  public boolean getOrRequestBoolean(String s, String description) {
    while (true) {
      ...
      if (fn.equals("true") || fn.equals("yes")) return (true);
      if (fn.equals("false") || fn.equals("no")) return (false);
    }
  }
  ...
  public void add(String key, String value) throws IOException {
    if (values == null || iniFile == null) throw new RuntimeException("Call init() before get()!");
    if (values.containsKey(key.toLowerCase())) return;
  ...
  * @throws ParameterFileException */
  ...
  public void init(File f, boolean mainIni) throws ParameterFileException {
    ...
    if (mainIni) {
      ...
      iniFile = f;
      if (basePath == null) basePath = (f.getParent() != null ? f.getParent() + "/" : "");
    }
    try {
      if (!iniFile.exists()) {
      ...
      for (String l : new FileLines(f)) {
        ...
        if (m.group(1).toLowerCase().equals("include")) {
          if (s.startsWith("/")) init(s, false);
          else init((f.getParent() != null ? f.getParent() + "/" : "") + s, false);
        } else {
  ...
  public void init(String filename, File... folders) throws ParameterFileException {
    ...
    for (File folder : folders) {
      if (new File(folder, filename).exists()) {
        if (found) throw new ParameterFileException("INI-file " + filename + "occurs twice in given folders");
  ...
  * @throws ParameterFileException */
  ...
  public Database getDatabase() throws SQLException, InstantiationException, IllegalAccessException, ClassNotFoundException {
    ...
    if (db == null) throw new RuntimeException("Unsupported database system " + system);
    if (fetchsize != null) db.setFetchsize(fetchsize);
    return db;
  }
